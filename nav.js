// ================================================================
//  nav.js — MENÚS POR ROL  3Villas  v15
// ================================================================
// VERSIÓN ACTUAL: v15 | Historial completo al final de este archivo
...
var _menuAdmin = [
  ...
  _itemEquipo,
  {
    ...
    children: [
      ...
      { label: 'Fichas de propietarios', url: 'fichas-propietarios.html',        icon: '📋' },
      { label: 'Control Waiver',      url: 'control-waiver.html',             icon: '🔧' },
      { label: 'Control Cobros',      url: 'cobros-inquilinos.html',          icon: '💶' },
      { label: 'Devoluciones pendientes', url: 'cobros-inquilinos.html?preset=devoluciones', icon: '↩️' },
  ...
  },
  {
    ...
    children: [
      { label: 'Test checkin-online',        url: 'checkin-testear-reserva.html',           icon: '🧪' },
...
(function () {
  var NAV_VERSION = 15; /* debe coincidir con la versión de este archivo */
  try {
...
// HISTORIAL: v15 - Nueva entrada 'Fichas de propietarios' (fichas-propietarios.html) en el bloque Administracion del menu de admin y manager, justo debajo de 'Nueva Villa'. Es la pagina del equipo que crea el enlace con el que un propietario rellena la ficha de su casa (pagina publica ficha-propietario.html) y que despues promueve esa ficha a una villa de TaVillas. Va al lado de 'Nueva Villa' porque las dos crean villas. La pagina declara Auth.require('crear-villa'), la clave que auth.js da SOLO a admin y manager (linea 131); no usa 'editar-villas', que la tienen tambien staff y sales, porque esta pagina crea enlaces con token y crea villas, y los dos unicos roles que pueden promover una ficha son admin y manager. El bloque Administracion solo esta en _menuAdmin, el menu de admin, manager y default, asi que la entrada la ven los mismos roles que ya ven 'Nueva Villa' y ningun rol ve un enlace que acabaria en 'Acceso denegado'. Ninguna clave de auth.js se toca. NAV_VERSION sube a 15 junto con la cabecera. Unico cambio. | v14 - La entrada 'Test checkin-online' del bloque Configuracion apuntaba a checkin-test-reserva.html, que no existe ni en el repo ni en el servidor (HTTP 404 comprobado el 14/09/2026). El archivo real se llama checkin-testear-reserva.html; en algun momento se renombro y el menu se quedo atras. Solo cambia la URL: la clave de permiso sigue siendo 'checkin-test-reserva', que es la que la propia pagina declara en su Auth.require y la que auth.js concede a admin. Unico cambio. | v13 - Fuera del menu de limpieza la entrada 'Villas' (buscar-villa.html): auth.js da la clave 'villas' solo a admin, manager, staff y sales, asi que a un rol cleaner esa entrada siempre acababa en 'Acceso denegado'. Misma correccion que la v9 hizo en el menu de sales, y ninguna clave de auth.js se toca. El menu de limpieza queda con Entradas y Salidas, Mis tareas y Login. Hoy mismo entradas-cleaner.html existe por fin (v6), asi que 'Entradas y Salidas' deja de llevar a un 404. | v12 - Gary (14/09/2026): quitadas del bloque Configuracion del menu de admin y manager las tres entradas 'Test1' (XXtest1XX.html), 'Test2' (XXtest2XX.html) y 'Test3' (XXtest3XX.html). Son copias antiguas de paginas vivas: XXtest1XX es entradas-equipo v96b (la viva va por v149), XXtest2XX es entradas-equipo v147 con la funcion sin terminar de limpiezas intermedias, y XXtest3XX es pedir-wellcomepacks v20 (la viva va por v21). XXtest2XX escribe, inserta y borra filas en Ta_limpiezas_intermedias con un solo confirm del navegador (lineas 2472, 2481, 2483), y su propio historial v141 registra que &method=PUT/DELETE dentro de encodeURIComponent hacia que Caspio INSERTara en vez de actualizar. Lectura de esa tabla por la API el 14/09/2026: 9 filas, 3 codigos de reserva, comentarios 'test siete', 'hola', 'ocho', 'diecisiete', nada nuevo desde el 14/07/2026 - son datos de prueba, sin uso real. Los tres archivos NO se borran en esta version y siguen accesibles por URL: su permiso es Auth.require('entradas'), que auth.js da a todos los roles, asi que quitar el enlace solo evita la entrada accidental desde el menu. Pendiente de decision de Jordi: si la funcion de limpiezas intermedias se sigue queriendo, va a la pagina viva; si no, se borra el archivo. Unico cambio de esta version. | v11 - Toni Segui (10/09/2026 18:43, WhatsApp): "que en el listado solo me salgan las reservas que tengo que devolver las fianzas". Control Cobros v34 trae ese listado hecho en un enlace (cobros-inquilinos.html?preset=devoluciones: checkout de hace 3 dias o antes, fianza sin devolver, solo las opciones 2 y 3), asi que entra en el menu como entrada propia "Devoluciones pendientes" en el bloque Administracion de admin y manager, justo debajo de Control Cobros, para que Toni no tenga que tocar ningun filtro. Unico cambio. | v10 - Toni Segui (10/09/2026 18:28, WhatsApp: "no encuentro control de cobros, ¿dónde está?"): la página Control Cobros (cobros-inquilinos.html, permiso admin y manager en auth.js) no estaba en ningún menú; solo se llegaba desde el enlace "Comprobar cobros" de la página de notas o por URL. Hoy esa página lleva los dos pasos de la devolución de fianza (v33) y Toni la necesita a mano: entra en el bloque Administración del menú de admin y manager, entre Control Waiver y Listado Guardias. Único cambio. | v9 - Menú de sales: quitados del bloque Administración los dos enlaces que auth.js no le permite abrir, 'Nueva Villa' (crear-villa.html) y 'Control Waiver' (control-waiver.html): auth.js da las dos páginas solo a admin y manager (líneas 130-131), así que para sales los dos enlaces acababan en 'Acceso denegado', y con ellos el botón de Mia cuando alguien de sales pedía esa página por su nombre (revisión de Mia del 2026-09-07, R10). No se toca ni un permiso: auth.js queda como estaba. Crear Villa es una página de ESCRITURA y no se amplía nunca; si Control Waiver (informe de solo lectura) tiene que verlo el equipo de ventas, es una decisión de Gary sobre auth.js y volvería a entrar en el menú con ella. El bloque Administración de sales se queda con 'Reuniones', que sí tiene permiso. Único cambio de esta versión; el resto de los menús queda igual. | v8 - FIX CRÍTICO: en el bloque 'Administración' del menú admin faltaba la coma entre los objetos 'Listado Guardias' y 'Reuniones' dentro del array children (probablemente un desliz al pegar la v7 en el servidor — el archivo que Jordi había compartido en el chat sí la tenía; el desplegado en el servidor no). Esa coma que faltaba causaba SyntaxError: Unexpected token '{' al parsear nav.js, lo que impedía que el archivo llegara a ejecutar la línea var NAV_MENU=... — como consecuencia, NAV_MENU quedaba undefined y nav-component.js pintaba el panel de menú completamente vacío (cabecera y pie sí se veían porque no dependen de NAV_MENU). De paso, corregido el icono de 'Listado Guardias', que en el archivo roto había quedado como 📅 (duplicando el de 'Reuniones') en vez de 🚨. | v7 - Auto-detección de versión nueva (mismo mecanismo "Opción B" ya implementado y probado en checkin-pasos.html desde su v76 y en reuniones.html desde su v14, pedido por Jordi para replicarlo aquí): nueva IIFE al final del archivo que hace un fetch de 'nav.js' con cache:'reload' (va a red saltándose la caché del navegador), lee el nuevo comentario "VERSIÓN ACTUAL: vXX" (añadido en la cabecera, junto al comentario de nombre/versión existente) de la copia fresca del servidor y lo compara con la constante embebida NAV_VERSION (actualizar en cada nueva versión junto con el número en la cabecera y en el historial). Si el servidor va por delante, recarga la página actual completa una sola vez por versión (marca antibucle en sessionStorage '3v_vchk_nav.js_v<XX>'); si va por detrás o falla (sin red, file://), no hace nada. A diferencia de las páginas HTML (que se recargan a sí mismas), aquí la recarga es de la página que esté cargando nav.js en ese momento — como nav.js lo cargan prácticamente todas las páginas del intranet, esto corrige de raíz el problema de menú desactualizado en TODO el sitio, no solo en una página. Coste: +1 fetch a nav.js por carga de página en todo el intranet (avisado y aceptado por Jordi). Recuerda: nav.js es de nombre fijo (excepción, como checkin-auth.js) — la versión va solo en la cabecera y el historial, nunca en el nombre del archivo. | v6 - Fix error de sintaxis en children de Configuración (comas faltantes); eliminado duplicado "Roles y permisos"; URL hostaway-comprobar-fechas-reservas.html añadida con .html | v5 - versión anterior
