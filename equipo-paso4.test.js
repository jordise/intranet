...
/* v151 cambio el nombre: el enlace ya no vive en urlEcFix sino en nerUrl, que
   ademas devuelve cadena vacia a quien no puede abrir la ficha. El destino es
   el mismo, y esto lo comprueba. */
ok('v151: el lapiz de ecotasa abre la pagina de notas, no la DataPage antigua',
  /_ecFix=nerUrl\(code,'&ci=ecotasa'\)/.test(SRC)
  && /notas-equipo-reservas\.html\?TaBookings2021_FS_confirmation_code='\+encodeURIComponent\(code\)/.test(SRC));
ok('el historial de v149 explica el lapiz y la intranet antigua', /\| v149 - [^|]*intranet antigua[^|]*notas-equipo-reservas\.html/.test(SRC));
