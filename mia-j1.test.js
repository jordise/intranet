...
(function () {
  ...
  var z = withFilters({ guest: '', villa: null, stay_on: undefined });
  ok('campos vacíos no cuentan', z.r === true && z.f.code === CODE);
  var z2 = withFilters({ tipo: 'salida' });
  ok('el tipo solo ("qué salidas hay") sigue siendo la pregunta de siempre: no se inyecta', z2.r === false && z2.f.code === undefined);
...
(function () {
  ...
  ok('el cuerpo de la petición solo añade pages bajo FEAT.page (askBody)', /body:JSON\.stringify\(askBody\(q\)\)/.test(S) && /q:q, page:curPage\(\), today:todayISO\(\)/.test(S) && /FEAT\.page/.test(S.slice(S.indexOf('function askBody'), S.indexOf('function askBody')+600)));
})();
